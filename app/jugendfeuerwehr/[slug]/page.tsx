...
export async function generateStaticParams(): Promise<Array<{ slug: string }>> {
  return youthBrigades.map((b) => ({ slug: b.id }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const brigade = youthBrigades.find((b) => b.id === slug);
  if (!brigade) return {};
  return buildMetadata({
    title: brigade.name,
    description: `Gerätehaus der ${brigade.name}.`,
    path: `/jugendfeuerwehr/${slug}`,
  });
}

export default async function JugendfeuerwehrDetailPage({ params }: Props) {
  const { slug } = await params;
  const brigade = youthBrigades.find((b) => b.id === slug);
    ...
    <article className="py-16 lg:py-24">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        <nav aria-label="Breadcrumb" className="mb-10 text-sm text-neutral-500">
          <Link href="/jugendfeuerwehren" className="hover:text-fire-600 no-underline">
            Jugendfeuerwehren
          </Link>
